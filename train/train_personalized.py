...
class WurstCore(TrainingCore, DataCore, WarpCore):
    ...
    def setup_models(self, extras: Extras) -> Models:  # configure model
        ...
        if self.config.use_ddp and self.config.training:
            ...
            train_lora = DDP(
                train_lora, device_ids=[self.device], find_unused_parameters=True
            )
    ...
    def forward_pass(self, data: WarpCore.Data, extras: Extras, models: Models):
        ...
        with torch.no_grad():
            ...
            if flag_lr:
                noised, noise, target, logSNR, noise_cond, loss_weight = (
                    extras.gdf.diffuse(latents_lr, shift=1, loss_shift=1)
                )
            else:
                noised, noise, target, logSNR, noise_cond, loss_weight = (
                    extras.gdf.diffuse(latents, shift=1, loss_shift=1)
                )
            if not flag_lr:
                (
                    noised_lr,
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            ...
            pred = models.generator(
                ...
                noise_cond,
                reuire_f=False,
                lr_guide=(lr_enc_guide, lr_dec_guide) if not flag_lr else None,
                **conditions,
            )
    ...
    def backward_pass(
        ...
        if update:
            ...
            grad_norm = nn.utils.clip_grad_norm_(
                models.train_lora.module.parameters(), 1.0
            )
    ...
    def sample(self, models: Models, data: WarpCore.Data, extras: Extras):
        ...
        with torch.no_grad():
            ...
            if self.is_main_node:
                print(
                    "sampling results hr latent shape ",
                    latents.shape,
                    "lr latent shape",
                    latents_lr.shape,
                )
