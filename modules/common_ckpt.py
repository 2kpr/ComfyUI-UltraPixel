...
class AttnBlock_lrfuse_backup(nn.Module):
    def __init__(
        self, c, c_cond, nhead, self_attn=True, dropout=0.0, use_checkpoint=True
    ):
        super().__init__()
        ...
        self.fuse_mapper = nn.Sequential(nn.SiLU(), Linear(c_cond, c))
        self.use_checkpoint = use_checkpoint

    def forward(self, hr, lr):
        return checkpoint(
            self._forward, (hr, lr), self.paramters(), self.use_checkpoint
        )
...
class AttnBlock_lrfuse(nn.Module):
    ...
    def forward(self, hr, lr):
        return checkpoint(
            self._forward, (hr, lr), self.parameters(), self.use_checkpoint
        )
...
class Attention2D_extra(nn.Module):
    ...
    def forward(self, x, kv, extra_emb=None, self_attn=False):
        ...
        if extra_emb is not None:
            ...
            extra_emb = extra_emb.view(
                extra_emb.size(0), extra_emb.size(1), -1
            ).permute(0, 2, 1)
...
class AttnBlock_extraq(nn.Module):
    ...
    def forward(self, x, kv, extra_emb=None):
        ...
        if extra_emb is not None:
            res_x, res_extra = self.attention(
                self.norm(x),
                kv,
                extra_emb=self.norm2(extra_emb),
...
class AttnBlock_crossbranch(nn.Module):
    ...
    def forward(self, x, kv, main_x):
        ...
        x = (
            self.channelwise1(
                torch.cat(
                    (x, F.interpolate(main_x.float(), x.shape[2:])), dim=1
                ).permute(0, 2, 3, 1)
            ).permute(0, 3, 1, 2)
            + x
        )
        ...
        main_x = (
            self.channelwise2(
                torch.cat(
                    (main_x, F.interpolate(x.float(), main_x.shape[2:])), dim=1
                ).permute(0, 2, 3, 1)
            ).permute(0, 3, 1, 2)
            + main_x
        )
...
class ResBlock(nn.Module):
    def __init__(
        self, c, c_skip=0, kernel_size=3, dropout=0.0, use_checkpoint=True
    ...
    def forward(self, x, x_skip=None):
        ...
        if x_skip is not None:
            return checkpoint(
                self._forward_skip, (x, x_skip), self.parameters(), self.use_checkpoint
            )
        else:
            # print('in line 298', x.shape)
            return checkpoint(
                self._forward_woskip, (x,), self.parameters(), self.use_checkpoint
            )
...
class AttnBlock(nn.Module):
    ...
    def forward(self, x, kv):
        return checkpoint(
            self._forward, (x, kv), self.parameters(), self.use_checkpoint
        )
